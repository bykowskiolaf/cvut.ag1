...
std::pair<bool, std::vector<Vertex>> topsort(const Graph& G) {
    size_t n = G.vertices();
    std::vector<int> in_degree(n, 0); // Array to store vertex in-degrees
    std::vector<Vertex> topological_order; // To store the topological order
    std::queue<Vertex> queue; // Queue for vertices with in-degree 0
    ...
    if (topological_order.size() == n) {
        ...
        std::function<bool(Vertex)> dfs = [&](Vertex v) -> bool {
            visited[v] = true;
            in_stack[v] = true;
            ...
            for (Vertex neighbor : G[v]) {
                if (!visited[neighbor]) {
                    ...
                    // Cycle found, reconstruct the cycle
        ...
        // This point should not be reached if a cycle exists
        return {false, {}};
    }
}
