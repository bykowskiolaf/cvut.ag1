...
enum Vertex : size_t { NO_VERTEX = -size_t(1), ROOT = -size_t(2) };
...
struct Graph {
  ...
  const std::vector<Vertex> &operator[](Vertex v) const {
    CHECK(size_t(v) < _adj.size(), "Graph: index %zu out of range [0..%zu).",
          size_t(v), _adj.size());
...
size_t bfs(const Graph &G, Vertex u, std::vector<Vertex> &P,
           std::vector<size_t> &D) {
  size_t visited = 0;
  std::queue<Vertex> Q;
  P[u] = ROOT;
...
void test_bfs_inner(const Graph &G, Vertex u) {
  ...
  for (Vertex v : G) {
    if (P[v] == NO_VERTEX) {
      CHECK(D[v] == NO_DISTANCE,
            "P[%zu] == NO_VERTEX but D[%u] == %zu not NO_DISTANCE.", size_t(v),
            size_t(v), D[v]);
    ...
    if (v != u) {
      CHECK(P[v] < G.vertices(), "P[%zu] == %zu >= %zu (# of vertices).",
            size_t(v), size_t(P[v]), G.vertices());
      CHECK(P[P[v]] != NO_VERTEX, "P[%zu] == %zu but P[%zu] == NO_VERTEX.",
            size_t(v), size_t(P[v]), size_t(P[v]));
    }

    for (Vertex w : G[v]) {
      CHECK(D[w] <= D[v] + 1,
            "D[%zu] == %zu but its neighbor has D[%zu] == %zu.", size_t(w),
            D[w], size_t(v), D[v]);
      ...
      CHECK(D[w] == D[v] + 1,
            "P[%zu] == %zu but D[%zu] == %zu != D[%zu] + 1 == %zu", size_t(w),
            size_t(v), size_t(w), D[w], size_t(v), 1 + D[v]);
      pred_ok[w] = true;
    }
  }
  ...
  CHECK(seen_r == seen_t,
        "Reported size of component is %zu but it should be %zu.", seen_t,
        seen_r);
}
